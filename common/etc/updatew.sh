...
curl -sS -k -L "${GITHUB_PROXY}https://raw.githubusercontent.com/CNMan/dnscrypt-proxy-config/refs/heads/master/public-resolvers.md" \
  -o "$DNSCRYPT_CONF_DIR/public-resolvers.md"

# 5.10 IP 白名单
curl -sS -k -L "${GITHUB_PROXY}https://raw.githubusercontent.com/CNMan/dnscrypt-proxy-config/refs/heads/master/parental-control.md" \
  -o "$DNSCRYPT_CONF_DIR/parental-control.md"

# 5.11 IP 白名单
curl -sS -k -L "${GITHUB_PROXY}https://raw.githubusercontent.com/CNMan/dnscrypt-proxy-config/refs/heads/master/odoh-servers.md" \
  -o "$DNSCRYPT_CONF_DIR/odoh-servers.md"

# 5.12 IP 白名单
curl -sS -k -L "${GITHUB_PROXY}https://raw.githubusercontent.com/CNMan/dnscrypt-proxy-config/refs/heads/master/odoh-relays.md" \
  -o "$DNSCRYPT_CONF_DIR/odoh-relays.md"

...
# 发布固件名称添加日期
sed -i 's|^IMG_PREFIX\:\=.*|IMG_PREFIX:=IM-192.168.1.2-$(shell TZ=UTC-8 date +"%Y.%m.%d-%H%M")-$(IMG_PREFIX_VERNUM)$(IMG_PREFIX_VERCODE)$(IMG_PREFIX_EXTRA)$(BOARD)$(if $(SUBTARGET),-$(SUBTARGET))|g' include/image.mk

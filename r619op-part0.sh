...
sed --follow-symlinks -i "s#%C\"#%C by R619AC'\"#" package/base-files/files/etc/os-release
...
sed -i "s/OpenWrt/R619AC/g" package/base-files/files/bin/config_generate package/base-files/image-config.in package/network/config/wifi-scripts/files/lib/wifi/mac80211.uc config/Config-images.in Config.in include/u-boot.mk include/version.mk || true
...
sed -i 's/^IMG_PREFIX\:\=.*/IMG_PREFIX:=R619AC-$(shell TZ=UTC-8 date +"%Y.%m.%d-%H%M")-$(IMG_PREFIX_VERNUM)$(IMG_PREFIX_VERCODE)$(IMG_PREFIX_EXTRA)$(BOARD)$(if $(SUBTARGET),-$(SUBTARGET))/g' include/image.mk

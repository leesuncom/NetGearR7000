...
curl -sS https://www.cloudflare.com/ips-v4/ > r619ac/etc/smartdns/ip-set/cloudflare-ipv4.txt
curl -sS https://raw.githubusercontent.com/Cats-Team/AdRules/main/smart-dns.conf > common/etc/smartdns/address.conf
curl -sS https://www.cloudflare.com/ips-v4/ > common/etc/smartdns/ip-set/cloudflare-ipv4.txt
...
echo "spki_cloudflare: $(echo | openssl s_client -connect '1.0.0.1:853' -servername cloudflare-dns.com 2> /dev/null | openssl x509 -pubkey -noout | openssl pkey -pubin -outform der | openssl dgst -sha256 -binary | openssl enc -base64)" > r619ac/etc/smartdns/spki
# 获取 8.8.8.8 的 SPKI 并追加到文件（不覆盖原有内容）
echo "spki_google: $(echo | openssl s_client -connect '8.8.8.8:853' -servername dns.google 2> /dev/null | openssl x509 -pubkey -noout | openssl pkey -pubin -outform der | openssl dgst -sha256 -binary | openssl enc -base64)" >> r619ac/etc/smartdns/spki
...
echo "spki_cloudflare: $(echo | openssl s_client -connect '1.0.0.1:853' -servername cloudflare-dns.com 2> /dev/null | openssl x509 -pubkey -noout | openssl pkey -pubin -outform der | openssl dgst -sha256 -binary | openssl enc -base64)" > common/etc/smartdns/spki
# 获取 8.8.8.8 的 SPKI 并追加到文件（不覆盖原有内容）
echo "spki_8.8.8.8: $(echo | openssl s_client -connect '8.8.8.8:853' -servername dns.google 2> /dev/null | openssl x509 -pubkey -noout | openssl pkey -pubin -outform der | openssl dgst -sha256 -binary | openssl enc -base64)" >> common/etc/smartdns/spki
echo "spki_google: $(echo | openssl s_client -connect 'https://dns.google/dns-query' 2> /dev/null | openssl x509 -pubkey -noout | openssl pkey -pubin -outform der | openssl dgst -sha256 -binary | openssl enc -base64)" >> common/etc/smartdns/spki
...
echo "spki_119.29.29.29: $(echo | openssl s_client -connect '119.29.29.29:853' 2> /dev/null | openssl x509 -pubkey -noout | openssl pkey -pubin -outform der | openssl dgst -sha256 -binary | openssl enc -base64)" >> common/etc/smartdns/spki
